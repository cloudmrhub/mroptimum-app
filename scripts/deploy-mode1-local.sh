...
#   VPC_ID              - VPC ID for ECS tasks (e.g., vpc-xxxxxxxxxxxxx)
...
# Get AWS account (preflight -- detect expired or missing credentials early)
echo "Checking AWS credentials..."
if ! ACCOUNT_ID=$(aws sts get-caller-identity $AWS_ARGS --query Account --output text 2>/dev/null); then
    echo ""
    echo "❌ Unable to get AWS caller identity. This commonly means your AWS credentials are missing, invalid, or expired." >&2
    echo "Suggested fixes:" >&2
    echo "  - If you use AWS SSO: run 'aws sso login --profile <profile>' and then re-run this script." >&2
    echo "  - If you use environment credentials: ensure AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, and AWS_SESSION_TOKEN (if applicable) are exported." >&2
    echo "  - If you use a named profile: export AWS_PROFILE=<profile> or set it before calling the script." >&2
    echo "To verify your credentials run: aws sts get-caller-identity --profile <profile>" >&2
    echo "" >&2
    exit 1
fi
...
# Write a small env file with Mode1 outputs so callers can source it
EXPORTS_FILE="exports.mode1.sh"
cat > "$EXPORTS_FILE" <<EOF
# Auto-generated by scripts/deploy-mode1-local.sh
export AWS_ACCOUNT_ID="$ACCOUNT_ID"
export STATE_MACHINE_ARN="$STATE_MACHINE_ARN"
export DATA_BUCKET="$DATA_BUCKET"
export FAILED_BUCKET="$FAILED_BUCKET"
export RESULTS_BUCKET="$RESULTS_BUCKET"
EOF

echo "Wrote Mode1 env file: $EXPORTS_FILE"
echo "To load these values into your shell, run: source $EXPORTS_FILE"
echo ""
echo "Note: this script now only deploys the SAM stack and writes $EXPORTS_FILE." 
echo "Registration is handled separately by scripts/register-mode1.sh (the wrapper will call it)."
echo "Done!"