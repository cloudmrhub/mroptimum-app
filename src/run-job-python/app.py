...
# Update the failed bucket and result bucket references:
mroptimum_failed = os.getenv("FailedBucketName", "mro-failed-dev-us-east-1")
mroptimum_result = os.getenv("ResultsBucketName", "mro-results-dev-us-east-1")
...
def handler(event, context,s3=None):
    ...
    try:
        ...
        if "token" in J:
            token = J["token"]  # Assign token from J
        else:
            raise Exception("Token not found in input JSON")
        
        # Ensure the pipelineid is assigned from JSON
        if "pipeline" in J:
            pipelineid = J["pipeline"]  # Assign pipelineid from J
        else:
            raise Exception("Pipeline ID not found in input JSON")
        ...
        try:
            ...
            if fj:
                print(f"couldn't copy {fj.getPosition()}")
            else:
                print("fj is not initialized or available")
